...
# Global variables
n = 1024  # Default value, can be updated
fps = 60  # Default value, can be updated
f = 0  # Global frame counter, can be updated
...
def rngi(min_val: int | None = None, max_val: int | None = None):
    if min_val is None and max_val is None:
        return random.randint(-sys.maxsize, sys.maxsize)
    elif min_val is None and max_val is not None:
        return random.randint(0, max_val)
    elif min_val is not None and max_val is None:
        return random.randint(0, int(min_val))
    elif min_val is not None and max_val is not None:
        return random.randint(min_val, max_val)
    # Fallback case, though the logic above should cover all scenarios
    return 0
...
def smooth_damp_advanced(
    current, target, current_velocity, smooth_time, max_speed, delta_time, fs=30
):
...
def sin01(p=1, w=1, off=0, size=None):
    if size is None:
        size = n
    t = np.linspace(0, tau * size / fps / p, size)
...
def range1(start, finish):
    ...
    if isinstance(start, str):
        raise NotImplementedError("String time parsing not implemented yet")
